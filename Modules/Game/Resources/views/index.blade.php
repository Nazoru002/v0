...
@section('card-title', 'Data Game')

@section('card-action')
  <a href="{{ route('game.create') }}" class="btn btn-primary btn-xs">
      ...
      <tbody>
        @forelse ($games as $item)
          <tr>
            ...
            <td class="align-middle text-center">
              <div class="btn-group">
                <a href="{{ route('game.edit', $item->id) }}" class="btn btn-sm btn-warning borad">
                  <span class="fa fa-edit align-middle"></span>
                </a>
                <form action="{{ route('game.active', $item->id) }}" method="post">
                  @csrf
                  @method('PUT')
